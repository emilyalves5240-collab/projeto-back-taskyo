...
export async function listar(req, res) {
  try {
    ...
    const empresas = await db.all(
      'SELECT id, nome, email, telefone FROM empresas ORDER BY id'
    );
    res.json(empresas);
  } catch (erro) {
    console.error('[empresa.listar]', erro);
...
export async function buscarPorId(req, res) {
  ...
  try {
    const db = await getDatabase();
    const empresas = await db.get(
      'SELECT id, nome, email, telefone FROM empresas WHERE id = ?',
      [id]
    );
...
export async function criar(req, res) {
  ...
  // entrada da empresa precisa passar por verificação no servidor.
  ...
  try {
    ...
    console.error('[empresa.criar]', erro);
    res.status(500).json({ mensagem: 'Erro ao salvar empresa.' });
  }
}

// PUT /empresa/:id — atualização parcial. Só o próprio empresa pode editar.
